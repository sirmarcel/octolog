...
# usage rake new_link['title','url']
desc "Create a new link post in #{source_dir}/#{posts_dir}"
task :new_link, [:title, :link_url] do |t, args|
  raise "### You haven't set anything up yet. First run `rake install` to set up an Octopress theme." unless File.directory?(source_dir)
  mkdir_p "#{source_dir}/#{posts_dir}"
  args.with_defaults(:title => 'new-post')
  title = args.title
...
task :rsync do
  ...
  ok_failed system("rsync -avze 'ssh -p #{ssh_port} -i /Users/Marcel/.ssh/sirmarcel' #{exclude} #{"--delete" unless rsync_delete == false} #{public_dir}/ #{ssh_user}:#{document_root}")
end
