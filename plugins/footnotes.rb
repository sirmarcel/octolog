	...
	class ReferenceTag < Liquid::Tag
		...
		def render(context)
			unless @reference.nil?
				"<sup id='fnref:#{@reference}'><a href='#fn:#{@reference}' class='ftnl' rel='footnote'>#{@reference}</a></sup>"
			end
		end
	end
...
<div class="footnotes">
		...
		def handle_fn_tag(params)
			ref_text = params
			@footnotes << "<p id='fn:#{@current_reference}'>#{ref_text}<a href='#fnref:#{@current_reference}' rev='footnote'>↩</a></p>"
			@current_reference += 1
		end
	end
end
